    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#datatable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{route('operator.api.supplier')}}"
                },
                columns: [{
                        data: 'id',
                        sortable: true,
            ...
            $('#supplierForm').on('submit', function (event) {
                ...
                if ($('#action').val() == 'Created') {
                    action_url = "{{route('operator.supplier.store')}}";
                }

                if ($('#action').val() == 'Updated') {
                    action_url = "{{route('operator.supplier.update', 'hidden_id->id')}}";
                }
            ...
            swal({
                ...
                preConfirm: function () {
                    return new Promise(function (resolve) {
                        $.ajax({
                                url: "{{ url('operator/supplier') }}" + '/' + supplierId,
                                type: "DELETE",
                                data: {
                                    '_method': 'DELETE',
                                    '_token': csrf_token
