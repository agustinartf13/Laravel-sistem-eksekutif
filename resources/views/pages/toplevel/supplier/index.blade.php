...
<div class="row">
    <div class="col-sm-12">
        <div class="page-title-box">
            ...
            <ol class="breadcrumb">
                <li>{{ Breadcrumbs::render('supptoplevel') }}</li>
            </ol>
        </div>
    </div>
</div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="card m-b-20">
                <div class="card-body">
                    <a
                    href="{{ route('toplevel.supplier.exportexcelsupplier') }}"
                    class="btn btn-success btn-flat d-inline"
                    style="float: right"
                    ><i class="fa fa-print"></i> Excel</a
                    >
                    <a
                    href="{{ route('toplevel.supplier.exportpdfsupplier') }}"
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#datatable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{route('toplevel.api.supplier')}}"
                },
                columns: [{
                        data: 'id',
                        sortable: true,
            ...
            $('#supplierForm').on('submit', function (event) {
                ...
                if ($('#action').val() == 'Created') {
                    action_url = "{{route('toplevel.supplier.store')}}";
                }

                if ($('#action').val() == 'Updated') {
                    action_url = "{{route('toplevel.supplier.update', 'hidden_id->id')}}";
                }
            ...
            swal({
                ...
                preConfirm: function () {
                    return new Promise(function (resolve) {
                        $.ajax({
                                url: "{{ url('toplevel/supplier') }}" + '/' + supplierId,
                                type: "DELETE",
                                data: {
                                    '_method': 'DELETE',
                                    '_token': csrf_token
